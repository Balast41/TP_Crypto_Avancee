...
// Fonction de setup qui va créer les éléments nécessaires au départ (=Setup dans BasIdent)
  public static SettingParameters setup(Pairing pairing){ // setup phase
    ...
    // Fonctionde keygen qui va créer une clé autour du String id
    ...
    // Fonction XOR, utilisée dans les autres fonctions
    public static byte[] Xor(byte[]a,byte[]b ){ //returns a Xor b 
        ...
        return new IBEcipher(U,V,aeskey.toBytes());
    }
    //Fonction d'encryption
    public static IBEcipher IBEencryption(IBEcipher AESKey, byte[] message) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException, UnsupportedEncodingException{
        
        byte[] Aescipher=AESCrypto.encrypt(message, AESKey.getAescipher());  // chiffrement AES
        
        return new IBEcipher(AESKey.getU(), AESKey.getV(), Aescipher); //instaciation d'un objet representant un ciphertext hybride combinant (BasicID et AES)
    }
